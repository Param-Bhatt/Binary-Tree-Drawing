...
/*! This function finds and evaluates all offsets for drawing the tree which 
are stored int the node defined in node class
*/
...
void traverse(node* root, int col) {
	if (root != NULL) {
		root->x = col;
		if (root->thread==true) {
			root->thread = false;
		...
		traverse(root->left, col - root->offset);
		traverse(root->right, col + root->offset);
	}
}
...
void tree::Start(void)
{
	...
	duration = (finish - end) / (double)CLOCKS_PER_SEC;
